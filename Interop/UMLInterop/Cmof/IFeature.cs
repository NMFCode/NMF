//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated by a tool.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using NMF.Collections.Generic;
using NMF.Collections.ObjectModel;
using NMF.Expressions;
using NMF.Expressions.Linq;
using NMF.Models;
using NMF.Models.Collections;
using NMF.Models.Expressions;
using NMF.Models.Meta;
using NMF.Models.Repository;
using NMF.Serialization;
using NMF.Utilities;
using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Collections.Specialized;
using System.ComponentModel;
using System.Diagnostics;
using System.Globalization;
using System.Linq;


namespace NMF.Interop.Cmof
{
    
    
    /// <summary>
    /// The public interface for Feature
    /// </summary>
    [DefaultImplementationTypeAttribute(typeof(Feature))]
    [XmlDefaultImplementationTypeAttribute(typeof(Feature))]
    [ModelRepresentationClassAttribute("http://www.omg.org/spec/MOF/20131001/cmof.xmi#//Feature")]
    public partial interface IFeature : IModelElement, IRedefinableElement
    {
        
        /// <summary>
        /// Specifies whether this feature characterizes individual instances classified by the classifier (false) or the classifier itself (true).
        /// </summary>
        [DefaultValueAttribute(false)]
        [TypeConverterAttribute(typeof(LowercaseBooleanConverter))]
        [DisplayNameAttribute("isStatic")]
        [DescriptionAttribute(("Specifies whether this feature characterizes individual instances classified by t" +
            "he classifier (false) or the classifier itself (true)."))]
        [CategoryAttribute("Feature")]
        [XmlElementNameAttribute("isStatic")]
        [XmlAttributeAttribute(true)]
        bool IsStatic
        {
            get;
            set;
        }
        
        /// <summary>
        /// Gets fired when the IsStatic property changed its value
        /// </summary>
        event EventHandler<ValueChangedEventArgs> IsStaticChanged;
        
        /// <summary>
        /// Gets fired before the IsStatic property changes its value
        /// </summary>
        event EventHandler<ValueChangedEventArgs> IsStaticChanging;
    }
}
