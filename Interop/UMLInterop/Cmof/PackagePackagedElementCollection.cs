...
namespace NMF.Interop.Cmof
{
    ...
    public class PackagePackagedElementCollection : ICollectionExpression<IPackageableElement>, ICollection<IPackageableElement>, IListExpression<IPackageableElement>, IList<IPackageableElement>
    {
        ...
        public virtual IPackageableElement this[int index]
        {
            ...
            set
            {
                ...
                if ((this._parent.OwnedType.Count 
                            <= (index - runningIndex)))
                {
                    this._parent.OwnedType[(index - runningIndex)] = ((NMF.Interop.Cmof.IType)(value));
                }
                else
                {
                    runningIndex = (runningIndex + this._parent.OwnedType.Count);
        ...
        public virtual int IndexOf(IPackageableElement item)
        {
            int runningIndex = 0;
            int index;
            IPackage itemAsPackage = item.As<IPackage>();
            if (itemAsPackage == null)
            {
                index = -1;
            }
            else
            {
                index = this._parent.NestedPackage.IndexOf(itemAsPackage);
            }
            if ((index == -1))
            {
                runningIndex = (runningIndex + this._parent.NestedPackage.Count);
            ...
            IType itemAsType = item.As<IType>();
            if (itemAsType == null)
            {
                index = -1;
            }
            ...
            {
                index = this._parent.OwnedType.IndexOf(itemAsType);
            }
            if ((index == -1))
            {
                runningIndex = (runningIndex + this._parent.OwnedType.Count);
        ...
        public virtual void Insert(int index, IPackageableElement item)
        {
            ...
            if ((this._parent.OwnedType.Count 
                        <= (index - runningIndex)))
            {
                this._parent.OwnedType.Insert((index - runningIndex), ((NMF.Interop.Cmof.IType)(item)));
                return;
            }
            else
            {
