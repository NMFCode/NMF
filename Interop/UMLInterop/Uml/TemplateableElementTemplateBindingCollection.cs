...
namespace NMF.Interop.Uml
{
    ...
    public class TemplateableElementTemplateBindingCollection : ObservableOppositeOrderedSet<ITemplateableElement, ITemplateBinding>
    {
        ...
        private void OnItemParentChanged(object sender, ValueChangedEventArgs e)
        {
            ...
            {
                this.Remove(((ITemplateBinding)(sender)));
            }
        }
        
        protected override void SetOpposite(ITemplateBinding item, ITemplateableElement parent)
        {
            if ((parent != null))
            {
                item.ParentChanged += this.OnItemParentChanged;
                item.BoundElement = parent;
            }
            else
            {
                item.ParentChanged -= this.OnItemParentChanged;
                if ((item.BoundElement == this.Parent))
                {
                    item.BoundElement = parent;
                }
            }
