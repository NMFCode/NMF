...
namespace NMF.Interop.Uml
{
    ...
    public class PropertyQualifierCollection : ObservableOppositeOrderedSet<IProperty, IProperty>
    {
        ...
        protected override void SetOpposite(IProperty item, IProperty parent)
        {
            if ((parent != null))
            {
                item.ParentChanged += this.OnItemParentChanged;
                item.AssociationEnd = parent;
            }
            else
            {
                item.ParentChanged -= this.OnItemParentChanged;
                if ((item.AssociationEnd == this.Parent))
                {
                    item.AssociationEnd = parent;
                }
            }
