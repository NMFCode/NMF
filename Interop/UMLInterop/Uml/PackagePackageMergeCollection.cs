...
namespace NMF.Interop.Uml
{
    ...
    /// </summary>
    public class PackagePackageMergeCollection : ObservableOppositeOrderedSet<IPackage, IPackageMerge>
    {
        ...
        public PackagePackageMergeCollection(IPackage parent) : 
                base(parent)
        {
        }
        ...
        private void OnItemParentChanged(object sender, ValueChangedEventArgs e)
        {
            ...
            {
                this.Remove(((IPackageMerge)(sender)));
            }
        }
        ...
        protected override void SetOpposite(IPackageMerge item, IPackage newParent)
        {
            if ((newParent != null))
            {
                item.ParentChanged += this.OnItemParentChanged;
                item.ReceivingPackage = newParent;
            }
            else
            {
                item.ParentChanged -= this.OnItemParentChanged;
                if ((item.ReceivingPackage == this.Parent))
                {
                    item.ReceivingPackage = newParent;
                }
            }
