...
namespace NMF.Interop.Uml
{
    ...
    /// </summary>
    public class InterfaceOwnedOperationCollection : ObservableOppositeOrderedSet<IInterface, NMF.Interop.Uml.IOperation>
    {
        ...
        protected override void SetOpposite(NMF.Interop.Uml.IOperation item, IInterface newParent)
        {
            if ((newParent != null))
            {
                item.ParentChanged += this.OnItemParentChanged;
                item.Interface = newParent;
            }
            else
            {
                item.ParentChanged -= this.OnItemParentChanged;
                if ((item.Interface == this.Parent))
                {
                    item.Interface = newParent;
                }
            }
