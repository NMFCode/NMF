    ...
    public partial class Meta2ClassesTransformation
    {
        ...
        public class RefinedAttributeCollectionClassGenerator : TransformationRule<IClass, IAttribute, CodeTypeDeclaration>
        {
            ...
            public override void Transform(IClass scope, IAttribute attribute, CodeTypeDeclaration generatedType, ITransformationContext context)
            {
                ...
                if (constraintValues.Any())
                {
                    CodeExpression[] initializers = new CodeExpression[constraintValues.Count];
                    for (int i = 0; i < constraintValues.Count; i++)
                    {
                        initializers[i] = CodeDomHelper.CreatePrimitiveExpression(constraintValues[i], elementType, attribute.Type is IEnumeration);
                    }
            ...
            protected virtual void ImplementNotifications(CodeTypeDeclaration generatedType, List<IAttribute> implementingAttributes, CodeConstructor constructor, CodeArgumentReferenceExpression constrParentRef, ITransformationContext context)
            {
                ...
                {
                    var attProperty = context.Trace.ResolveIn(Rule<Attribute2Property>(), att);
                    if (att.UpperBound == 1)
                    {
                        if (single == null)
                        {
            ...
            protected virtual void ImplementList(CodeTypeDeclaration generatedType, CodeTypeReference elementType, CodeExpression standardValuesRef, List<IAttribute> implementingAttributes, List<string> constraintValues, ITransformationContext context)
            {
                ...
                generatedType.Members.Add(GenerateIndexOf(implementingAttributes, constraintValues, elementType, standardValuesRef, context));
                generatedType.Members.Add(GenerateInsert(implementingAttributes, constraintValues, elementType, context));
                generatedType.Members.Add(GenerateRemoveAt(implementingAttributes, constraintValues, context));
                generatedType.Members.Add(GenerateIndexer(implementingAttributes, constraintValues, elementType, standardValuesRef, context));
            }
            ...
            protected virtual void ImplementCollection(CodeTypeDeclaration generatedType, CodeTypeReference elementType, CodeExpression standardValuesRef, List<IAttribute> implementingAttributes, List<string> constraintValues, ITransformationContext context)
            {
                ...
                generatedType.Members.Add(GenerateAdd(implementingAttributes, elementType, context));
                generatedType.Members.Add(GenerateClear(implementingAttributes, context));
            ...
            private CodeMemberMethod GenerateAdd(IEnumerable<IAttribute> implementingAttributes, CodeTypeReference elementType, ITransformationContext context)
            {
                ...
                {
                    var attribRef = GetAttributeReference(attrib, context);
                    if (attrib.UpperBound == 1)
                    {
                        var ifNull = new CodeConditionStatement
                        {
                            Condition = new CodeBinaryOperatorExpression(attribRef, CodeBinaryOperatorType.ValueEquality, new CodePrimitiveExpression(null))
                        };
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Condition = new CodeBinaryOperatorExpression(new CodePropertyReferenceExpression(attribRef, "Count"), CodeBinaryOperatorType.LessThan, new CodePrimitiveExpression(attrib.UpperBound))
                            };
            ...
            private CodeMemberMethod GenerateClear(IEnumerable<IAttribute> implementingAttributes, ITransformationContext context)
            {
            ...
            private CodeExpression GetAttributeReference(IAttribute attrib, ITransformationContext context)
            {
                return new CodePropertyReferenceExpression(parentRef, context.Trace.ResolveIn(Rule<Attribute2Property>(), attrib).Name);
            }

            private CodeMemberMethod GenerateContains(IEnumerable<IAttribute> implementingAttributes, List<string> constraintValues, CodeTypeReference elementType, CodeExpression standardValuesRef, ITransformationContext context)
            {
            ...
            private CodeMemberMethod GenerateCopyTo(IEnumerable<IAttribute> implementingAttributes, List<string> constraintValues, CodeTypeReference elementType, CodeExpression standardValuesRef, ITransformationContext context)
            {
                ...
                if (constraintValues.Any())
                {
                    ...
                    copyTo.Statements.Add(new CodeAssignStatement(arrayIndexRef, new CodeBinaryOperatorExpression(arrayIndexRef, CodeBinaryOperatorType.Add, new CodePrimitiveExpression(constraintValues.Count))));
                }
                foreach (var attrib in implementingAttributes)
                {
                    var attribRef = GetAttributeReference(attrib, context);
                    if (attrib.UpperBound == 1)
                    {
                        var ifNull = new CodeConditionStatement
                        {
                            Condition = new CodeBinaryOperatorExpression(attribRef, CodeBinaryOperatorType.IdentityInequality, new CodePrimitiveExpression(null))
                        };
            ...
            private CodeMemberProperty GenerateCount(IEnumerable<IAttribute> implementingAttributes, List<string> constraintValues, ITransformationContext context)
            {
                ...
                count.GetStatements.Add(new CodeVariableDeclarationStatement(typeof(int), "count", new CodePrimitiveExpression(constraintValues.Count)));
            ...
            private CodeMemberMethod GenerateRemove(IEnumerable<IAttribute> implementingAttributes, CodeTypeReference elementType, ITransformationContext context)
            {
            ...
            private CodeMemberMethod GenerateGenericGetEnumerator(IEnumerable<IAttribute> implementingAttributes, List<string> constraintValues, CodeTypeReference elementType, CodeExpression standardValuesRef, ITransformationContext context)
            {
                ...
                foreach (var attrib in implementingAttributes)
                {
                    currentExpression = new CodeMethodInvokeExpression(currentExpression, "Concat", GetAttributeReference(attrib, context));
                }
            ...
            private CodeMemberMethod GenerateIndexOf(IEnumerable<IAttribute> implementingAttributes, List<string> constraintValues, CodeTypeReference elementType, CodeExpression standardValuesRef, ITransformationContext context)
            {
                ...
                if (constraintValues.Count > 0)
                {
                    ...
                    iterateStandards.TestExpression = new CodeBinaryOperatorExpression(iRef, CodeBinaryOperatorType.LessThan, new CodePrimitiveExpression(constraintValues.Count));
                    ...
                    runningIndexInit = constraintValues.Count;
                }
                ...
                foreach (var attrib in implementingAttributes)
                {
                    ...
                    if (attrib.UpperBound == 1)
                    {
                        ...
                        {
                            Condition = new CodeBinaryOperatorExpression(itemRef, CodeBinaryOperatorType.ValueEquality, attribRef)
                        };
            ...
            private CodeMemberMethod GenerateInsert(IEnumerable<IAttribute> implementingAttributes, List<string> constraintValues, CodeTypeReference elementType, ITransformationContext context)
            {
                ...
                insert.Statements.Add(new CodeVariableDeclarationStatement(typeof(int), "runningIndex", new CodePrimitiveExpression(constraintValues.Count)));
                var runningIndexRef = new CodeVariableReferenceExpression("runningIndex");
            ...
            private CodeMemberMethod GenerateRemoveAt(IEnumerable<IAttribute> implementingAttributes, List<string> constraintValues, ITransformationContext context)
            {
                ...
                removeAt.Statements.Add(new CodeVariableDeclarationStatement(typeof(int), "runningIndex", new CodePrimitiveExpression(constraintValues.Count)));
                var runningIndexRef = new CodeVariableReferenceExpression("runningIndex");
                var indexRef = new CodeArgumentReferenceExpression("index");
                foreach (var attrib in implementingAttributes)
                {
            ...
            private CodeMemberProperty GenerateIndexer(IEnumerable<IAttribute> implementingAttributes, List<string> constraintValues, CodeTypeReference elementType, CodeExpression standardValuesRef, ITransformationContext context)
            {
                ...
                if (constraintValues.Count > 0)
                {
                    indexer.GetStatements.Add(new CodeConditionStatement(new CodeBinaryOperatorExpression(indexRef, CodeBinaryOperatorType.LessThan, new CodePrimitiveExpression(constraintValues.Count)),
